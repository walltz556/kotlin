...
private class DelegatingFlowImpl(
    ...
    override fun getTopFlow(): DelegatingFlow {
        return DelegatingFlowImpl(null, approvedInfos, conditionalInfos)
    }

    private inline fun collect(block: (DelegatingFlowImpl) -> Unit) {
...
abstract class DelegatingLogicSystem(private val context: DataFlowInferenceContext) {
    ...
    fun mergeFlows(flows: Collection<DelegatingFlow>): DelegatingFlow {
        ...
        variablesFromApprovedInfos.forEach { variable ->
            val infos = approvedInfosFromAllFlows.mapNotNull { it[variable] }
            if (infos.size != flows.size) return@forEach
            val intersectedInfo = or(infos)
            if (intersectedInfo.isEmpty) return@forEach
